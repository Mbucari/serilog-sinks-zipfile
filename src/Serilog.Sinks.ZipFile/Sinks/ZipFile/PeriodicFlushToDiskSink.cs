...
namespace Serilog.Sinks.ZipFile
{
    ...
    public class PeriodicFlushToDiskSink : ILogEventSink, IDisposable
    {
        ...
        public PeriodicFlushToDiskSink(ILogEventSink sink, TimeSpan flushInterval)
        {
            ...
            if (sink is IFlushableZipFileSink flushable)
            {
            ...
            else
            {
                ...
                SelfLog.WriteLine("{0} configured to flush {1}, but {2} not implemented", typeof(PeriodicFlushToDiskSink), sink, nameof(IFlushableZipFileSink));
            }
        }
        ...
        void FlushToDisk(IFlushableZipFileSink flushable)
        {
            try
            {
